...
router.post('/jobs/:id/pickup', authenticate, async (req, res) => {
  try {
    ...
    const blockedUntilPickup = user.freelancerPickupBlockedUntil ? new Date(user.freelancerPickupBlockedUntil) : null;
    if (blockedUntilPickup && blockedUntilPickup.getTime() > Date.now()) {
      return res.status(403).json({
        success: false,
        ...
        blockedUntil: blockedUntilPickup,
      });
    }
...
router.post('/jobs/:id/apply', authenticate, async (req, res) => {
  try {
    ...
    await clearExpiredFreelancerPickupBlock(user);
    const blockedUntilApply = user.freelancerPickupBlockedUntil ? new Date(user.freelancerPickupBlockedUntil) : null;
    if (blockedUntilApply && blockedUntilApply.getTime() > Date.now()) {
      return res.status(403).json({
        success: false,
        error: 'You cannot apply for jobs right now. Please try again later.',
        code: 'FREELANCER_PICKUP_BLOCKED',
        reason: 'support_cancel_unassign_cooldown',
        blockedUntil: blockedUntilApply,
      });
    }
...
router.post('/jobs/:id/offer', authenticate, async (req, res) => {
  try {
    ...
    await clearExpiredFreelancerPickupBlock(user);
    const blockedUntilOffer = user.freelancerPickupBlockedUntil ? new Date(user.freelancerPickupBlockedUntil) : null;
    if (blockedUntilOffer && blockedUntilOffer.getTime() > Date.now()) {
      return res.status(403).json({
        success: false,
        error: 'You cannot make offers right now. Please try again later.',
        code: 'FREELANCER_PICKUP_BLOCKED',
        reason: 'support_cancel_unassign_cooldown',
        blockedUntil: blockedUntilOffer,
      });
    }

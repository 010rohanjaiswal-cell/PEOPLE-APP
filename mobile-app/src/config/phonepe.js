...
export const startPhonePeTransaction = async (params) => {
  try {
    ...
    // checkSum is REQUIRED by the native module
    // The native code checks: if (checkSum.isEmpty() || body.isEmpty()) throw error
    // So we must validate and provide a non-empty checksum
    if (!checkSum || checkSum.trim().length === 0) {
      throw new Error('Checksum is required for PhonePe SDK transaction. It should be generated by the backend and cannot be empty.');
    }
    ...
    try {
      // PhonePe SDK ACTUAL native signature (from PhonePePaymentSDKModule.kt):
      // startTransaction(body: String, checkSum: String, packageName: String?, appSchema: String?)
      // 
      // The native module requires 4 parameters:
      // - body: JSON string containing orderId, merchantId, token, paymentMode (REQUIRED)
      // - checkSum: Checksum generated by backend (REQUIRED - cannot be empty!)
      // - packageName: Optional package name (can be null)
      // - appSchema: Optional app schema for deep linking (can be null for Android, required for iOS)
      //
      // Note: The official documentation shows 2 parameters, but the actual native implementation requires 4.
      // The checksum is REQUIRED and checked for emptiness in the native code.
      
      if (!checkSum || checkSum.trim().length === 0) {
        throw new Error('Checksum is required and cannot be empty. The native module validates this.');
      }
      
      const packageName = null; // Optional package name
      
      console.log('📱 Calling startTransaction with actual native signature (4 parameters):', {
      ...
      if (Platform.OS === 'android') {
        // For Android: body, checkSum, packageName (null), appSchema (null)
        console.log('📱 Android: Calling with 4 parameters (body, checkSum, null, null)');
        ...
        console.log('📱 iOS: Calling with 4 parameters (body, checkSum, null, appSchema)');

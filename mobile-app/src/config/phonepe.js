...
export const startPhonePeTransaction = async (params) => {
  try {
    ...
    // Note: checkSum is not used in the 2-parameter version per official documentation
    // The SDK may handle checksum internally or it might not be needed for SDK orders
    // Keeping checkSum parameter for backward compatibility but not using it
    ...
    try {
      // PhonePe SDK official documentation signature:
      // startTransaction(request: string, appSchema: string | null)
      // 
      // According to official docs, only 2 parameters are needed:
      // - request: JSON string containing orderId, merchantId, token, paymentMode
      // - appSchema: Optional (not needed for Android), required for iOS
      //
      // Note: The checksum might be handled internally by the SDK or included in the request body
      // We're trying the 2-parameter version as per official documentation
      
      console.log('📱 Calling startTransaction with official 2-parameter signature:', {
      ...
      if (Platform.OS === 'android') {
        // For Android: appSchema is not needed (can be null)
        console.log('📱 Android: Calling with 2 parameters (request, null)');
        response = await PhonePe.startTransaction(requestBodyString, null);
      } else {
        // For iOS: appSchema is required
        console.log('📱 iOS: Calling with 2 parameters (request, appSchema)');
